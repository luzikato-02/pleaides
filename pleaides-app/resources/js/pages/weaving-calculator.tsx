...
import { AlertTriangle, CheckCircle, Circle, Save, SlidersHorizontal, TrendingDown, TrendingUp } from 'lucide-react';
...
import { store } from '@/routes/weaving-calculations';
...
// Compact labelled input — smaller height, tighter label
function F({
    label,
    value,
    onChange,
...
const EMPTY: Record<string, string[]> = { ao: ['', ''], ai: ['', '', '', ''], bi: ['', '', '', ''], bo: ['', ''] };
...
export default function WeavingCalculator({
    ...
    const [productionOrder, setPO]          = useState('');
    const [loomNumber, setLoom]             = useState('');
    const [weavingStyle, setStyle]          = useState('');
    const [bodyTension, setBodyTension]     = useState('');
    const [twistingLength, setTwisting]     = useState('');

    // Creel params
    const [totalRolls, setRolls]            = useState('');
    const [totalProduced, setProduced]      = useState('');
    const [warpEnds, setWarpEnds]           = useState('');
    const [tubeTypeId, setTubeTypeId]       = useState('');
    const [stdRollLen, setStdLen]           = useState('');
    const [adjLen, setAdj]                  = useState('');
    const [gpm1, setGpm1]                   = useState('');
    const [gpm2, setGpm2]                   = useState('');

    // Length adjustment events 2 & 3
    ...
    const handleSave = () => {
        ...
        router.post(store.url(), {
            ...
            // Snapshots
            standard_creel_length_snapshot: machine?.standard_creel_length ?? null,
            quality_factor_snapshot:        machine?.quality_factor ?? null,
            tolerance_kg_snapshot:          machine?.tolerance_kg ?? null,
            // Length adjustment events
    ...
    return (
        <>
            <Head title="Weaving Calculator" />

            <div className="flex flex-col gap-3 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-xl font-semibold">Weaving Waste Calculator</h1>
...
WeavingCalculator.layout = {
    breadcrumbs: [{ title: 'Weaving Calculator', href: '/weaving-calculator' }],
};
